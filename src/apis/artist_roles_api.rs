...
pub type ArtistRolesGetError = StandardApiError;

/// struct for typed errors of method [`artist_roles_id_get`]
pub type ArtistRolesIdGetError = StandardApiError;


/// Retrieves multiple artistRoles by available filters, or without if applicable.
pub async fn artist_roles_get(configuration: &configuration::Configuration, filter_id: Option<Vec<String>>) -> Result<models::ArtistRolesMultiResourceDataDocument, Error<ArtistRolesGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_filter_id = filter_id;

    let uri_str = format!("{}/artistRoles", configuration.base_path);
...
pub async fn artist_roles_id_get(configuration: &configuration::Configuration, id: &str) -> Result<models::ArtistRolesSingleResourceDataDocument, Error<ArtistRolesIdGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ArtistRolesSingleResourceDataDocument`")))),
        }
    } else {
        let content = resp.text().await?;
