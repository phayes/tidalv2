...
pub type ArtworksGetError = StandardApiError;

/// struct for typed errors of method [`artworks_id_get`]
pub type ArtworksIdGetError = StandardApiError;

/// struct for typed errors of method [`artworks_id_relationships_owners_get`]
pub type ArtworksIdRelationshipsOwnersGetError = StandardApiError;

/// struct for typed errors of method [`artworks_post`]
pub type ArtworksPostError = StandardApiError;
...
pub async fn artworks_id_relationships_owners_get(configuration: &configuration::Configuration, id: &str, include: Option<Vec<String>>, page_cursor: Option<&str>) -> Result<models::ArtworksMultiRelationshipDataDocument, Error<ArtworksIdRelationshipsOwnersGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_include = include;
    let p_page_cursor = page_cursor;

    let uri_str = format!("{}/artworks/{id}/relationships/owners", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ArtworksIdRelationshipsOwnersGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Creates a new artwork.
pub async fn artworks_post(configuration: &configuration::Configuration, artwork_create_operation_payload: Option<models::ArtworkCreateOperationPayload>) -> Result<models::ArtworksSingleResourceDataDocument, Error<ArtworksPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
